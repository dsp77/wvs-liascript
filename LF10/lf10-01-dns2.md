...
## Rekursive Abfrage mit DNSSEC
 ...
 1. Vor der eigentlichen DNS-Anfrage an den Root-Server fragt der DNS-Resolver den **DNSKEY** des Root-Servers ab. Der Schlüssel ist signiert und kann mit dem [**Root-Trust-Anchor**](https://data.iana.org/root-anchors/root-anchors.xml), einem öffentlichen Schlüssel, der im DNS-Resolver hinterlegt ist, überprüft werden.
 2. Nachdem auf die Anfrage an den Root-Server der Verweis auf den `a.nic.de` geliefert wurde, wird mit eingeschaltetem DNSSEC auch das **DS-Record** für `a.nic.de` geliefert.
 3. Als Nächstes erfolgt die Anfrage an den `a.nic.de`. Hier wird zunächst der **DNSKEY** des Servers abgefragt, dessen Hashwert mit dem vorigen **DS-Record** geliefert wurde. Der Resolver bildet jetzt selbst den Hashwert über den empfangenen Schlüssel und vergleicht ihn mit dem vom Root-Server gelieferten Hashwert. Sind beide gleich, ist die Authentizität des Schlüssels sichergestellt. 