<!--
author:   Günter Dannoritzer
email:    g.dannoritzer@wvs-ffm.de
version:  1.0.2
date:     14.09.2026
language: de
narrator: Deutsch Female

comment:  Domain Name System (DNS) - Zweiter Teil

icon:    https://raw.githubusercontent.com/dsp77/wvs-liascript/0938e2e0ce751e270e3e36b8ecfeb09044a41aa0/wvs-logo.png
logo:     02_img/logo-dns2.jpg

tags:     LiaScript, Domain Name System, DNS, Server, DoT

link:     https://cdn.jsdelivr.net/chartist.js/latest/chartist.min.css

script:   https://cdn.jsdelivr.net/chartist.js/latest/chartist.min.js

attribute: Lizenz: [CC BY-SA](https://creativecommons.org/licenses/by-sa/4.0/)
-->
# Domain Name System (DNS) - Zweiter Teil

Diese Information baut auf dem ersten Teil [DNS](https://liascript.github.io/course/?https://raw.githubusercontent.com/dsp77/wvs-liascript/main/LF03/dns.md) im Lernfeld 3 auf.


## DNS-Records

DNS-Records (Domain Name System Records) sind spezifische Einträge in der DNS-Datenbank, die festlegen, wie eine Domain und deren Subdomains aufgelöst werden sollen. Sie enthalten wichtige Informationen darüber, welche IP-Adresse oder Dienste einer bestimmten Domain zugeordnet sind. Diese Einträge ermöglichen es, dass Benutzer Domains (wie beispiel.de) anstelle von IP-Adressen (wie 192.0.2.1) verwenden können.  

### Wichtige DNS-Record-Typen:

1. **A-Record (Address Record)**  
   - Verknüpft eine Domain mit einer IPv4-Adresse.  
   - Beispiel: `example.com -> 93.184.216.34`

2. **AAAA-Record (IPv6 Address Record)**  
   - Verknüpft eine Domain mit einer IPv6-Adresse.  
   - Beispiel: `example.com -> 2606:2800:220:1:248:1893:25c8:1946`

3. **CNAME-Record (Canonical Name Record)**  
   - Leitet eine Domain oder Subdomain auf eine andere Domain weiter.  
   - Beispiel: `www.example.com -> example.com`

4. **MX-Record (Mail Exchange Record)**  
   - Gibt den Mailserver für den Empfang von E-Mails an.  
   - Beispiel: `example.com -> mail.example.com`

5. **TXT-Record (Text Record)**  
   - Speichert frei definierbaren Text, oft für Sicherheits- und Verifizierungszwecke, wie SPF, [DKIM](https://liascript.github.io/course/?https://raw.githubusercontent.com/dsp77/wvs-liascript/main/LF11/lf11-19-digitale-signatur-anwendung.md#3) oder Google-Site-Verifikation.  
   - Beispiel: `example.com -> "v=spf1 include:example.net -all"`

6. **NS-Record (Name Server Record)**  
   - Gibt an, welche Nameserver für die Domain zuständig sind.  
   - Beispiel: `example.com -> ns1.example.com`

7. **PTR-Record (Pointer Record)**  
   - Verknüpft eine IP-Adresse mit einer Domain, verwendet für Reverse-DNS-Lookups.  
   - Beispiel: `192.0.2.1 -> example.com`

8. **SRV-Record (Service Record)**  
   - Dient zur Angabe von Diensten, Ports und Protokollen für bestimmte Anwendungen.  
   - Beispiel: `_sip._tcp.example.com -> 0 5 5060 sipserver.example.com`

9. **SOA-Record (Start of Authority Record)**  
   - Enthält administrative Informationen über die Domain, wie den primären Nameserver und die Kontaktadresse.  
   - Beispiel: `ns1.example.com -> admin@example.com`

### Wofür werden DNS-Records verwendet?

- **Domainauflösung**: Übersetzung von Domainnamen in IP-Adressen.  
- **E-Mail-Routing**: Weiterleitung von E-Mails an den richtigen Mailserver (über MX-Records).  
- **Subdomain-Verwaltung**: Einrichtung von Subdomains für spezifische Dienste.  
- **Sicherheitskonfiguration**: Implementierung von SPF, DKIM und DMARC für E-Mails.  
- **Service-Routing**: Festlegen von spezifischen Diensten (z. B. VoIP, XMPP).

### DNS-Records in Filius

Die folgende Abbildung wurde bereits im ersten Teil verwendet, um einem Domain-Namen eine IPv4-Adresse zuzuordnen. Neben A-Records unterstützt der DNS-Server von Filius noch MX-Records für E-Mail und NS-Records für die Delegation zu anderen DNS-Server.

![DNS-Record im Filius-DNS-Server konfigurieren](../LF03/02_img/lf03_dns_add_a_record.png)

## Filius Übung `NS-Record`
Die [Filius-Datei zu NS-Records](03_filius/lf10-01-dns-subdomain.fls) kann über den Link heruntergeladen werden.

![Übersicht NS-Record-Übung](02_img/lf10-01-dns-sub-domain-filius.png)

Der Netzwerkplan zeigt zwei Server `www.domain.de` und `www.sub.domain.de`, die als Webserver vom Notebook erreichbar sind. Das Netzwerk ist ein Minimalbeispiel, um die Funktion von NS-Records zu erklären. In größeren Netzwerkstrukturen wird die Verwaltung der Namensserver auf mehrere Administratorinnen oder Administratoren verteilt. Diese Delegation wird mithilfe von NS-Records durchgeführt.

Für die Domäne `domain.de` ist der Namensserver `dns.de` autoritativ. Für den Server der Subdomain `sub.domain.de` ist der DNS-Server `sub.dns.de` autoritativ. Da die Anfrage zuerst an den Namensserver `dns.de` gestellt wird, muss über das NS-Record eine Delegation der Anfragen für die Subdomain `sub.domain.de` an den DNS-Server `sub.dns.de` eingerichtet werden.


### DNS-Konfiguration

Für die Namensauflösung soll der Server `dns.de` die Auflösung für den Webserver `www.domain.de` übernehmen und der Server `sub.dns.de` die Auflösung für den Webserver `www.sub.domain.de` übernehmen.

 * `dns.de`

   * **A-Record**:

     * Domain: `www.domain.de`
    * IP-Adresse: `192.168.0.20`

 * `sub.dns.de`
  
    * **A-Record**:

     * Domain: `www.sub.domain.de`
    * IP-Adresse: `192.168.0.21`


Alle Anfragen werden an den DNS-Server `dns.de` geleitet. Damit der Server die Namensauflösung für die Subdomain an den Server `sub.dns.de` weiterleitet, wird mit einem `NS-Record` eine **Delegation** eingerichtet.

 * `dns.de`

   * **NS-Record**: 
    
     * Domain: `sub.domain.de`
    * Nameserver: `sub.dns.de`

   * **A-Record**:
    
     * Host/Domainname: `sub.dns.de`
    * IP-Adresse: `192.168.0.11`

### Client- und Server-Konfiguration

Auf den beiden Webservern `www.domain.de` und `www.sub.domain.de` wird ein Webserver installiert.

Auf dem Notebook wird ein Webbrowser installiert.

### Aufgabe

Rufen Sie mit dem Notebook folgende Webseiten auf und beobachten Sie, die Signalisierung der Netzwerkverbindung (grünes blinken):

 * `www.domain.de`
 * `www.sub.domain.de`

Von welchem DNS-Server hat NB die IP-Adresse für `www.domain.de` erhalten:

 * [(x)] `dns.de`
 * [( )] `sub.dns.de`

Von welchem DNS-Server hat NB die IP-Adresse für `www.sub.domain.de` erhalten:

 * [( )] `dns.de`
 * [(x)] `sub.dns.de`

## Filius Übung `MX-Record`

Für die Einrichtung eines E-Mail-Servers werden zwei Notebooks an das Netzwerk angeschlossen. Die folgende Abbildung zeigt das erweiterte Netzwerk. Der E-Mail-Server `mail.domain.de` wird über den DNS-Server `dns.de` aufgelöst.

![Übersicht Mail-Server-Übung](./02_img/lf10-01-dns-mailserver.png)

Die zugehörige [Filius-Datei](03_filius/lf10-01-dns-mailserver.fls) kann üaber den Link heruntergeladen werden.

### DNS-Konfiguration

Die folgende Konfiguration ist für den Mailversand nötig.

* `dns.de`

   * **MX-Record**: 
    
     * Maildomain: `domain.de`
    * Domainname Mailserver: `mail.domain.de`

   * **A-Record**:
    
     * Host/Domainname: `mail.domain.de`
    * IP-Adresse: `192.168.0.20`


### Client-/Server-Konfiguration

 * `mail.domain.de`

   * E-Mail-Server installieren
   * E-Mail-Konten erstellen

   * Maildomain: `domain.de`

       * Benutzername: `user1`, `user2`

Den Server starten.

  * NB1

    ![Mail-Client konfigurieren](02_img/lf10-01-mail-konto-konfigurieren.png)

 * NB2

    * Konfigurieren Sie analog zu NB1 einen `user2` auf NB2

### Aufgabe: Eine Maildomäne

Rufen Sie das E-Mail-Konto ab und beobachten Sie dabei die Signalisierung der Netwerkverbindung (grünes blinken). Der `mail.domain.de`-Server sollte erreicht werden.

In dem DNS-Server ist **kein** `MX-Record` konfiguriert. Kann der `user1@domain.de` eine E-Mail an `user2@domain.de` senden?

- [(x)] Ja, denn ein `MX-Record` ist nur nötig, wenn E-Mails zu anderen E-Mail-Domänen gesendet werden müssen.
- [( )] Nein, denn auch für E-Mails innerhalb einer E-Mail-Domäne muss der SMTP-Server den Namen des Posteingangsservers per `MX-Record erfragen. 

### Mehrere E-Mail-Server

In dem vorherigen Szenario ist nur ein E-Mail-Server in Verwendung und die Nutzung von `MX-Records` ist in dem Einserver-Szenario nicht richtig erkennbar. In der folgenden Abbildung werden drei E-Mail-Server verwendet und um von einer E-Mail Domain in eine andere E-Mails zu versenden, sind die `MX-Records`notwendig.

![E-Mail-Server](02_img/lf10-01-dns-email-mx-records.png)

Die zugehörige [Filius-Datei mit E-Mail-Servern](./lf10-01-dns-email-mx-records.fls) kann über den Link heruntergeladen werden.

### Aufgabe: Mehrere Mail-Domänen

 - Rufen Sie mit jedem E-Mail-Client empfangene E-Mails vom jeweiligen Server ab.
 - Überprüfen Sie den Netzwerkmitschnitt einer Verbindung. Welches DNS-Record wird vor der Anfrage an den E-Mail-Server vom DNS-Server angefragt?

   - [(x)] A-Record des E-Mail-Servers
   - [( )] NS-Record des zugehörigen DNS-Servers
   - [( )] MX-Record der abgefragten E-Mail-Domäne.

- Senden Sie von `user1@gmail.com` eine E-Mail an `user1@yandex.ru`.
- Markieren Sie, welche Aussagen bezüglich des E-Mail-Versandes richtig sind:

   <!-- data-randomize -->
   - [[x]] Der E-Mail-Client für das Konto `user1@gmail.com` fragt den DNS-Server nach dem A-Record für `mail.gmail.com`.
   - [[ ]] Der E-Mail-Client für das Konto `user1@gmail.com` fragt den DNS-Server nach dem MX-Record für `mail.yandex.ru`.
   - [[ ]] Der E-Mail-Server `mail.yandex.ru` fragt den DNS-Server nach dem MX-Record für E-Mail-Domäne `gmail.com`.
   - [[x]] Der E-Mail-Server `mail.gmail.com` fragt den DNS-Server nach dem MX-Record für E-Mail-Domäne `yandex.ru`.

## DNS-Resolver und Abfrage in der DNS-Hierarchie

Die folgende Abbildung zeigt ein Firmennetzwerk, verbunden über einen Internet-Service-Provider mit dem Internet.

Die DNS-Hierarchie der Server ist reduziert auf einen DNS-Root-Server (a.rootservers.net), einen DENIC-Server für die Top-Level-Domäne .de (`a.nic.de`) und einen `dns.1und1.de`-Server, der für die Domäne `www.domain.de` zuständig ist.

![DNS-Abfrage in der DNS-Hierarchie](02_img/lf10-01-dns-firma-provider-dns-hierarchie.png)

Die zugehörige [Filius-Datei mit DNS-Abfrage in der DNS-Hierarchie](lf10-01-dns-firma-provider-dns-hierarchie.fls) kann über den Link heruntergeladen werden.

Der Client **NB1** ist konfiguriert, um den lokalen DNS-Server `dns.firma.local` zu verwenden. Dieser löst lokale Domain-Namen innerhalb des Firmennetzwerks auf. Sollte er keine Auflösung durchführen können, leitet er diese an den DNS-Resolver des Internet-Service-Provides `dns.telekom.de` weiter. Dieser führt eine **rekursive Abfrage** in der DNS-Hierarchie durch.

Die DNS-Hierarchie der autoritativen DNS-Server wird über sogenannte **Delegationen** mithilfe von **NS Records** konfiguriert. Abfragende Resolver, wie in dem Beispiel der `dns.telekom.de` starten die Abfrage bei einem der [13 DNS-Root-Server](https://www.iana.org/domains/root/servers). In dem Beispiel ist nur der `a.root-servers.net` dargestellt. Die anderen 12 Server mit den Buchstaben b bis m sind gleich konfiguriert und dienen der Lastverteilung und administrativen Trennung. Die Information über die 13 Root-Server erhält der Resolver über das sogenannte [**Root Hints File**](https://www.iana.org/domains/root/files), das in der Software des DNS-Resolver hinterlegt ist. Wenn Sie z.B. bei einem Windows-Server die DNS-Rolle hinzufügen, werden Sie bei der Ausführung des Konfigurationsassistenten gefragt, wie eine nicht auflösbare Domäne behandelt werden soll. Als Auswahl ist es möglich, einen externen Resolver als sogenannten **Forwarder** einzutragen oder **Root Hints** zu verwenden.

Die Delegation in der Hierarchie sieht wie folgt aus:

````
 a.root.server.net         -> de. NS a.nic.de
   |
   +- a.nic.de             -> domain.de NS dns.1und1.de
         |
         +- dns.1und1.de   -> www.domain.de A 217.160.0.119
````

Parallel zu der Delegation für die Top Level Domain **`de`** haben die Root-Server weitere Delegationen für alle vorhandenen Top Level Domains. Dies kann eingesehen werden in den sogenannten [**Root Zone File**](https://www.iana.org/domains/root/files).

Beachten Sie, dass die Delegation immer über den Namen des autoritativen DNS-Servers erfolgt. Das bedeutet, es muss immer noch ein **A** und für IPv6 ein **AAAA** Record eingetragen werden, damit eine Verbindung zu dem Server hergestellt werden kann.

### Aufgabe

Rufen Sie mit dem Webbrowser des Notebooks **NB1** die Webeseite `www.domain.de` auf.

 - Analysieren Sie den **Paketverkehr am NB1**. Wieviele DNS-Pakete sehen Sie für den Aufruf `www.domain.de`? [[4]]
 - Welches Record enthält das letzte DNS-Paket, dass Notebook 1 empfängt?

    - [(x)] `www.domain.de A 217.160.0.119`
    - [( )] `dns.1und1.de NS 49.13.203.53`
    - [( )] `a.nic.de NS 194.0.0.53`
    - [( )] `a.root-servers.net NS 198.41.0.4`

 -  Analysieren Sie die **DNS-Pakete am DNS-Resolver** `dns.telekom.de`. Wieviele DNS-Pakete sehen Sie hier, für die Auflösung `www.domain.de`? [[8]]

Anmerkung, mit der Software Filius kann kein **Forwarder** für den Resolver eingetragen werden. Daher führt der lokale Resolver `dns.firma.local` eine Delegation an den Resolver `dns.telekom.de` durch. Das führt zu einer Antwort des lokalen Resolvers an das Notbook und eine erneute Anfrage an den Resolver `dns.telekom.de`. Daher sehen Sie hier vier DNS-Pakete. Mit eine Forwarder würden nur zwei Pakete zu sehen sein.

Am Resolver sind acht Pakete zu sehen. Als erstes die Anfrage von NB1 und als letztes die Antwort an NB1. Dann kommen jeweils zwei Pakete pro autoritativen DNS-Server in der Hierarchie dazu. Das sind die Anfrage und die Antwort an den jeweiligen autoritativen Server in der Reihenfolge `a.root-servers.net`, `a.nic.de` und `dns.1und1.de`.

# DNS-Root-Server

Die oberste Ebene in der DNS-Hierarchie sind die DNS-Root-Server. Informationen über DNS-Root-Server findet man auf der Webseite https://root-servers.org/.

Die offizielle [Webseite der IANA für die Root-Server](https://www.iana.org/domains/root/servers) enthält neben der Liste der 13 Server mit IPv4- und IPv6-Adressen auch [Dateien](https://www.iana.org/domains/root/files), wie z.B.:

 - [Root-Hint](https://www.internic.net/domain/named.root)
 - [Root-Zone](https://www.internic.net/domain/root.zone)
 - [Root-Trust-Anchor](https://data.iana.org/root-anchors/root-anchors.xml)

Geschützt gegenüber DDOS-Angriffe werden die DNS-Root-Server durch IP-Anycast. In der [LF4-Einheit zu DDOS](https://liascript.github.io/course/?https://raw.githubusercontent.com/dsp77/wvs-liascript/main/LF04/ddos.md) wird das erklärt.

![Verweis auf LF4](02_img/verweis.png) [LF4-Einheit zu DDOS](https://liascript.github.io/course/?https://raw.githubusercontent.com/dsp77/wvs-liascript/main/LF04/ddos.md)

![Webseite über die DNS-Root-Server](02_img/lf10-01-dns-root-servers_org.png)

# DNS over HTTPS (DoH) und DNS over TLS (DoT)


![Übersicht DoH und DoT im Netzwerk](02_img/lf10-01-dns-doh-dot-im-netzwerk.png)

## DNS over HTTPS (DoH)

![DNS over HTTPS (DoH) im Firefox](02_img/lf10-01-doh-firefox.png)

## DNS over TLS (DoT)

In einem Internetzugangsrouter wird DNS over TLS aktiviert. In der folgenden Abbildung wird diese Konfiguration beispielhaft in einer FritzBox durchgeführt.

![DoT-Konfiguration für die FritzBox](./02_img/lf10-01-dot-fritzbox.png)

Als DNS-over-TLS-Server werden die folgenden drei Server konfiguriert.

 * `dns.quad9.net`
 * `cloudflare-dns.com`
 * `dns.google`

Die folgende Abbildung zeigt ein Netzwerk mit der durchgeführten Konfiguration. DNS-Abfragen durch den Internetzugangsrouter werden nicht an den DNS-Server des Internet-Service-Providers gesendet, sondern blau hinterlegt, an die konfigurierten DNS-over-TLS-Server über Port 853 verschlüsselt übertragen.

![DoT-Abfrage durch den Internetzugangsrouter](./02_img/lf10-01-dns-over-tls.png)

Clients, die per DHCP ihre IP von dem Internetzugangsrouter bekommen, erhalten diesen als DNS-Server. Der Client sendet dann unverschlüsselt DNS-Anfragen über UDP an den Port 53. Der Router leitet dann die Anfragen über Port 853 mit TLS verschlüsselt an den externen Resolver weiter.

Einer der ausgewählten DoT-Server führt dann die Abfrage in der DNS-Hierarchie durch.

# Domain Name System Security Extensions (DNSSEC)

![DNS Security Extensions (DNSSEC)](02_img/lf10-01-dnssec-im-netzwerk.svg)

## Weitere Records für DNSSEC

Für DNSSEC werden weitere Records eingeführt. Drei wichtige Records sind `RRSIG`, `DNSSEC` und `DS`.

| Record           | Bedeutung                                                                               |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **RRSIG**        | Enthält die digitale Signatur für ein Resource Record Set (z. B. alle A-Records eines Namens).                                                         |
| **DNSKEY**       | Enthält die öffentlichen Schlüssel der Zone. Es gibt zwei Typen: ZSK (Zone Signing Key) und KSK (Key Signing Key).                                     |
| **DS**           | „Delegation Signer“ – enthält den Hashwert eines DNSKEY-Records der untergeordeten Hierarch. Ein DNSKEY-Record in der Zone `beispiel.de` erhält z.B. ein entsprechendes DS-Record in der `.de`-Zone.          |


## Root-Zone-Datei

Das folgende Beispiel enthält einen Ausschnitt aus der Root-Zone-Datei, die unter https://www.iana.org/domains/root/files verfügbar ist.


```
01   de.			172800	IN	NS	a.nic.de.
02   de.			172800	IN	NS	f.nic.de.
03   de.			172800	IN	NS	l.de.net.
04   de.			172800	IN	NS	n.de.net.
05   de.			172800	IN	NS	s.de.net.
06   de.			172800	IN	NS	z.nic.de.
07   de.			86400	IN	DS	26755 8 2 F341357809A5954311CCB82ADE114C6C1D724A75C0395137AA3978035425E78D
08   de.			86400	IN	RRSIG	DS 8 1 86400 20260615050000 20260602040000 54393 . 
```

In den Zeilen 1-6 sind für die `.de`-Zone die NS-Records der Namensserver `a.nic.de`, `f.nic.de`, `l.nic.de`, `n.nic.de`, `s.nic.de` und `z.nic.de`.

Es folgt dann in Zeile 7 ein DS-Record für die Domäne `.de`, ein Hashwert für das DNSKEY-Record in den genannten Namensserver der `.de`-Zone.

In Zeile 8 ist schließlich mit einem RRSIG-Record die Signatur über das DS-Record aus Zeile 7 enthalten.

## Rekursive Abfrage ohne DNSSEC

Die rekursive Abfrage eines Resolvers nach `www.domain.de` mithilfe von DNSSEC läuft jetzt folgendermaßen ab.

![Rekursive Namensauflösung](02_img/lf10-01-dns-recursive-name-resolution.png)

 1. Der DNS-Resolver fragt nach der IPv4-Adresse für `www.domain.de`. Über [**Root-Hint**](https://www.internic.net/domain/named.root) kennt er die 13 DNS-Root-Server und wählt einen aus. In dem Fall wird der `a.root-server.net` angefragt. 
 2. Der Root-Server kann die Anfrage nicht beantworten, kennt aber die Namensserver, die für die Top-Level-Domäne .de. zuständig ist und senden einen Verweis (Referral) auf diese Server. Das Beispiel ist zur Übersichtlichkeit vereinfacht und es wird nur einer der Nameserver zurückgesendet. In dem Fall der `a.nic.de`.
 3. Der Resolver stellt die vorherige Anfrage jetzt an den `a.nic.de`. 
 4. Der `a.nic.de` kann ebenfalls den Namen nicht auflösen, kennt aber den **autoritativen** DNS-Server, der für die Domäne `domain.de` zuständig ist. Als Antwort sendet er den Namensserver `ns15.domserver.net`.
 5. Der Resolver stellt erneut die Anfrage, jetzt an den in der vorherigen Antwort erhaltenen DNS-Server.
 6. Der `ns15.domserver.net` ist autoritativ für die Anfrage und sendet das gespeicherte **A-Record** an den Resolver zurück.

## DNSSEC Konfiguration

Für DNSSEC werden die zuvor besprochenen Records hinzugefügt. Die folgende Abbildung zeigt die Konfiguration des Root- und des de-Servers.

![Konfiguration für DNSSEC](02_img/lf10-01-dnssec-config.png)

## Rekursive Abfrage mit DNSSEC

Die Abfrage mit aktivem DNSSEC ist zur Übersichtlichkeit unterteilt in die Abfrage an den DNS-Root-Server und an den DNS-DE-Server.

### Anfrage an den DNS-Root-Server

Die folgende Abbildung zeigt die Anfrage nach dem FQDN `www.domain.de` an den DNS-Root-Server
`a.root-servers.net`.

![Namensauflösung an den DNS-Root-Server mit Überprüfung](02_img/lf10-01-dnssec-abfrage-root.png)

 1. Vor der eigentlichen DNS-Anfrage an den Root-Server fragt der DNS-Resolver den **DNSKEY** des Root-Servers ab. 
 2. Der Schlüssel ist signiert und als Antwort werden mit dem **DNSKEY-Record** der Schlüssel und mit dem **RRSIG-Record** die Signatur des Records gesendet.
 3. Um die Vertrauenskette aufzubauen, überprüft der Resolver die empfangene Signatur mithilfe des [**Root-Trust-Anchors**](https://data.iana.org/root-anchors/root-anchors.xml), einem öffentlichen Schlüssel, der im DNS-Resolver hinterlegt ist.
 4. Jetzt erfolgt die eigentliche Anfrage zur Namensauflösung für `www.domain.de`.
 5. Als Antwort wird neben dem Verweis auf den `a.nic.de` auch das **DS-Record** für `a.nic.de` und die zugehörige Signatur mit dem **RRSIG-Record** geliefert.
 6. Mithilfe des unter (1.) gefragten öffentlichen Schlüssels kann jetzt die Signatur überprüft und die Vertrauenskette bis zum Hashwert des DNSKEY für den de‑Server erweitert werden.

### Anfrage an den DNS-DE-Server

Die folgende Abbildung zeigt die Anfrage nach dem FQDN `www.domain.de` an den DNS-DE-Server
`a.nic.de`.

 ![Anfrage an den de.-Server mit Überprüfung](02_img/lf10-01-dnssec-abfrage-de.png)

 1. Mit dem vom Root-Server erhaltenen Verweis auf `a.nic.de` erfolgt die Anfrage an den Server. Hier wird zunächst auch wieder der **DNSKEY** des Servers abgefragt.
 2. Der Schlüssel kommt als Antwort.
 3. Mithilfe des vom Root-Server erhaltenen Hashwerts des unter 2. empfangenen Schlüssels, kann dessen Authentizität überprüft werden. Der Resolver bildet jetzt selbst den Hashwert über den empfangenen Schlüssel und vergleicht ihn mit dem vom Root-Server gelieferten Hashwert. Sind beide gleich, ist die Authentizität des Schlüssels sichergestellt. Die Vertrauenskette wurde bis zum Schlüssel des de‑Servers erweitert.